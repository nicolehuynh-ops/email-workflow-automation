...
from pcs.campaign_config import load_single_chase_config
...
def main():
    ...
    candidate_rows = [
        {
            **row,
            "action": "Candidate for step 3",
            "reason": "Active, not replied, not bounced, not opted out, not OOO, issuer not blocked, currently step 2",
        }
    ...
    sequence_contacts = {
        ...
        for row, contact in zip(
            candidate_rows,
            reply.get_sequence_contacts_by_ids(sequence_id, [row["contactId"] for row in candidate_rows]),
        )
    }
    for row in candidate_rows:
        contact = sequence_contacts[row["contactId"]]
        if contact.get("emailAccountId") != campaign_config.sender_account_id:
            sender_failures.append(
                {
                    **row,
                    "action": "Not moved",
                    "reason": f"Sender check failed. Expected {campaign_config.sender_label} emailAccountId {campaign_config.sender_account_id}, got {contact.get('emailAccountId')}",
                }
            )
        else:
            eligible_rows.append(
                {
                    **row,
                    ...
                    "reason": f"Active, not replied, not bounced, not opted out, not OOO, issuer not blocked, currently step 2, sender verified as {campaign_config.sender_label}",
                }
            )
    ...
    if sender_failures:
        workbook_path = write_run_workbook(
            ...
            workbook_rows([*rsvp_matches, *sender_failures]),
            notes_with_sequence(
                [
                    f"No contacts were moved because at least one candidate was not assigned to {campaign_config.sender_label}.",
            ...
            output_dir=config.SINGLE_CHASE_OUTPUT_DIR,
        )
        raise RuntimeError(
            f"Blocked: {len(sender_failures)} candidates are not assigned to {campaign_config.sender_label}. Workbook: {workbook_path}"
        )
    ...
    if not dry_run:
        require_live_confirmation(
            SCRIPT_NAME,
            sequence_id,
            campaign_config.sender_label,
            campaign_config.sender_account_id,
            eligible_rows,
            finished_rows,
            confirm_send,
            rsvp_matches,
    ...
    moved_sequence_contacts = {
        row["contactId"]: contact
        for row, contact in zip(
            eligible_rows,
            reply.get_sequence_contacts_by_ids(sequence_id, [row["contactId"] for row in eligible_rows]),
        )
    }
    for row in eligible_rows:
        contact = moved_sequence_contacts[row["contactId"]]
        if contact.get("emailAccountId") != campaign_config.sender_account_id:
            verification_failures.append(
                {
                    **row,
                    "action": "Sender verification failed",
                    "reason": f"Expected {campaign_config.sender_label} emailAccountId {campaign_config.sender_account_id}, got {contact.get('emailAccountId')}",
                }
            )
